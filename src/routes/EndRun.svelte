<script lang="ts">
  ...
  import EndRunAchievements from "../parts/EndRunAchievements.svelte";
  import EndRunMilestoneHints from "../parts/EndRunMilestoneHints.svelte";
  ...
  $: eraProgress = eras.map(([key, name]) => {
    ...
    return {
      ...
      known:
        ...
        (key === "eterniaSilent29624" &&
          ($gameState.data.global.knowledge.includes(
            KNOWLEDGE.ETERNIA_SILENT29624.visited,
          ) ||
            SILENT_VISIT_ACTION_IDS.some((id) =>
              $gameState.data.global.completedActionHistory.includes(id),
            ))),
    };
  }).filter(({ known }) => known);
  $: knownEraKeys = new Set(eraProgress.map(({ key }) => key));
  $: milestoneHintCandidates = allMilestones.filter(
    ({ id, location }) =>
      knownEraKeys.has(location) &&
      !globallyReachedMilestoneIds.includes(id as MilestoneKey),
  );
...
<div class="end_run_layout grid h-full gap-1 overflow-hidden px-2 pb-2">
  ...
  >
    <EndRunAchievements />
    <EndRunMilestoneHints
      candidates={milestoneHintCandidates}
      discoveredCount={globallyReachedMilestoneIds.length}
